...
const MENU: {
  ...
  {
    // 목자는 담당 목장 페이지, 관리자·목회자는 전체 목장 목록
    href: (role) => role === "shepherd" ? "/my-group" : "/cell-groups",
    label: "목장",
    roles: ["admin", "pastor", "shepherd"],
    icon: (
      <svg width="16" height="16" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.75}>
...
export default function Sidebar({
  ...
  const visible = MENU.filter((m) => m.roles.includes(role)).map((m) => ({
    ...m,
    resolvedHref: typeof m.href === "function" ? m.href(role) : m.href,
  }));
      ...
      >
        {visible.map((item) => {
          const active =
            ...
            (item.resolvedHref !== "/dashboard" && pathname.startsWith(item.resolvedHref));
          return (
            <Link
